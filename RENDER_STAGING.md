...
| `SENTRY_DSN` | (2026-09-13) a real DSN from a real Sentry project the owner created. Verified working locally end-to-end before this was documented as done: `sentry_sdk.get_client().is_active()` returned `True` against the real DSN, and a genuine `sentry_sdk.capture_exception()` call returned a real event ID and flushed successfully. Still needs setting as an actual Render env var (this row documents the variable, not that it's deployed) — without it, `sentry_sdk.init(dsn=None, ...)` stays a documented no-op, same "safe when unset" posture as every other credential here. |
| `DJANGO_ADMIN_EMAILS` | (optional, added 2026-09-13) comma-separated real email addresses (e.g. `owner@example.com,oncall@example.com`) — Django's own, second, independent error channel: when `DEBUG=False`, every unhandled exception emails everyone here via whatever `EMAIL_BACKEND` is configured. Still a no-op until a real `EMAIL_BACKEND` exists too (see the "Real email provider" item above) — set both together. |
...
| `GEMINI_MODEL` | (optional, default `gemini-3.6-flash` — was `gemini-2.5-flash` until 2026-09-12, when Google deprecated it; see the default's own comment in `config/settings/base.py`) |
...
| `GROQ_CHAT_MODEL` | (optional, default `openai/gpt-oss-20b` — was `llama-3.3-70b-versatile` until 2026-09-12, when it moved to Groq's Enterprise-only tier; see the default's own comment in `config/settings/base.py`) |
| `AI_CHAT_DAILY_LIMIT` | (optional, default `15`) free messages per user per day before an upgrade-to-Pro prompt (429). A `Subscription`-active (Spekooh Plus) user skips this cap entirely — see `PaperChatView`'s own comment. |
...
| `TWILIO_ACCOUNT_SID` / `TWILIO_API_KEY_SID` / `TWILIO_API_KEY_SECRET` | (2026-09-14, optional) real Twilio credentials — the API Key SID/Secret pair (Twilio Console → Account → API keys & tokens), not the Account Auth Token; Twilio's REST API accepts either for Basic Auth. Without all three, `apps.core.sms` raises `SMSUnavailable` (caught, not a crash) — phone verification and SMS notifications both just stay off. |
| `TWILIO_VERIFY_SERVICE_SID` | (optional) a Verify Service SID (Twilio Console → Verify → Services) — powers `apps.accounts`'s phone verification (`/api/auth/verify-phone/`, `/api/auth/verify-phone/confirm/`). Confirmed live (2026-09-14): needs no purchased phone number, works even on a $0-balance Trial account. |
| `TWILIO_MESSAGING_FROM_NUMBER` | (optional) a real purchased Twilio phone number or Messaging Service — a **separate** requirement from Verify above. Without it, `apps.notifications.services.notify(sms=True)` no-ops safely; the in-app notification still gets created either way. |
