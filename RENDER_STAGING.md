...
| `SENTRY_DSN` | (optional, still unset here as of 2026-09-13 — a real release-roadmap P0 blocker, not a staging gap) a real DSN from your own Sentry project (free tier exists). Without it, `sentry_sdk.init(dsn=None, ...)` is a documented no-op — zero server-side error visibility, found the hard way once already (see `SECURITY.md`'s "Known gaps"). |
